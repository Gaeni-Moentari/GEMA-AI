...
class Tasks:
    ...
    def Booksearch(self):
        if "GBOOK GAENI" in self.Input.upper():
            return Task(
                description=f"""
                    Menjawab pertanyaan tentang Gbook Gaeni. Berikan penjelasan mengenai apa itu Gbook Gaeni, 
                    termasuk fitur dan manfaatnya.
                """,
                expected_output=f"""
                    Penjelasan mengenai Gbook Gaeni yang mencakup:
                    - Apa itu Gbook Gaeni?
                    - Fitur utama yang ditawarkan.
                    - Manfaat menggunakan Gbook Gaeni untuk pengguna.
                    - Link resmi untuk mengakses Gbook Gaeni.
                """,
                agent=Agents().Contextualize()
            )
        else:
            return Task(
                description=f"""
                    Gunakan [GbookTool] untuk mencari informasi buku melalui API resmi Gbook Gaeni di endpoint:
                    https://gbook.gaeni.org/api-v1/book, berdasarkan input "{self.Input}".
                """,
                expected_output=f"""
                    Informasi buku yang mencakup:
                    - Judul Buku.
                    - ID Buku.
                    - Penulis.
                    - Kategori.
